    ...
    def load_data(self, csv_path: Path) -> None:
        ...
        for _, row in tqdm(df.iterrows(), total=len(df), desc="Parsing juegos"):
            try:
                ...
                platforms = _parse_list_field(row.get("platforms", ["Windows"]))

                pos_ratio = float(row.get("positive_ratio", 0) or 0)
                tot_rev   = int(row.get("total_reviews", 0) or 0)

                # Rating derivado: pos_ratio ponderado por volumen (0-10)
                if tot_rev > 0:
                    confidence = min(1.0, np.log10(tot_rev + 1) / 4)
                    raw_rating = pos_ratio * 10
                ...
                game = Game(
                    app_id        = int(row.get("app_id", row.get("appid", 0))),
                    ...
                    platforms     = platforms,
                    image_url     = str(row.get("header_image", "") or ""),
                )
